...
int main(int argc, char **argv) {
    ...
    struct ip_totals totals[MAX_IPS];
    int n_totals = 0;
    ...
    if (kernel_total_rx > 0) {
        ...
        printf("Unattributed RX = %" PRIu64 " (%.4f%% of kernel RX)\n",
               (kernel_total_rx > sum_ips_rx) ? (kernel_total_rx - sum_ips_rx) : 0ULL, pct_rx);
    }
    if (kernel_total_tx > 0) {
        ...
        printf("Unattributed TX = %" PRIu64 " (%.4f%% of kernel TX)\n",
               (kernel_total_tx > sum_ips_tx) ? (kernel_total_tx - sum_ips_tx) : 0ULL, pct_tx);
    }
